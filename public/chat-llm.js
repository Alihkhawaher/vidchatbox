...
const ChatLLM = (function() {
    ...
    async function* generateStream(message, captions, modelId, onProgress, history = []) {
        ...
        if (Array.isArray(history)) {
            for (const item of history.slice(-20)) {
                if (item && ['user', 'assistant'].includes(item.role) && typeof item.content === 'string' && item.content.trim()) {
        ...
        try {
            if (isBonsai(mId)) {
                ...
                yield { type: 'final', content: '', fullContent, aborted: Boolean(abortController?.signal?.aborted) };
                ...
                yield { type: 'final', content: '', fullContent: fullContent, aborted: Boolean(abortController?.signal?.aborted) };
            }

        } catch (err) {
            if (err.name === 'AbortError' || abortController?.signal?.aborted) {
                yield { type: 'final', content: '', fullContent: '', aborted: true };
            } else {
                throw err;
            }
        } finally {
