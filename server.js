...
// Configure CORS — only allow same-origin requests plus an explicit
// whitelist from ALLOWED_ORIGINS (comma-separated, e.g. "https://vidchatbox.example.com,http://192.168.1.10:3005")
const allowedOrigins = (process.env.ALLOWED_ORIGINS || '')
    .split(',')
    .map(o => o.trim())
    .filter(Boolean);
// CORS: reflect the origin only when it is same-origin (origin host matches the request
// Host header — works for any LAN IP/domain) or explicitly whitelisted via ALLOWED_ORIGINS.
// Cross-origin browser requests from other websites get no CORS headers and are blocked by the browser.
app.use((req, res, next) => {
    const origin = req.headers.origin;
    if (!origin) return next(); // same-origin or non-browser request — nothing to do
    try {
    ...
    } catch { /* invalid origin header — ignore */ }
    next();
});
...
// Base64-encoded audio for STT can exceed 50MB (38MB MP3 → ~51MB base64 JSON)
// 2-hour video at 128kbps MP3 ≈ 115MB raw → ~153MB base64 → 200MB limit for safety
