...
class GoogleProvider extends ProviderUtils.BaseProvider {
    ...
    static async generateResponse(message, captions, userApiKey = null, history = []) {
        try {
            const apiKey = await this.getValidKey(userApiKey);

            const contents = [
                ...ProviderUtils.sanitizeHistory(history).map(item => ({
            ...
            const response = await axios.post(
                ...
                {
                    systemInstruction: { parts: [{ text: ProviderUtils.prepareSystemPrompt(captions) }] },
                    contents
                },
                {
                    headers: { 'Content-Type': 'application/json' }
